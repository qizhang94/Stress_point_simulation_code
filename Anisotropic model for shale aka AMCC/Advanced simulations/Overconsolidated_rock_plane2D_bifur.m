...
% Investigate the trend of plane stran compression test (xOz plane)
Props = [1.07, 0.0026, 0, 0]; % M, lambda, kappa, theta (rad)
...
comp_incr = -1e-5; % Compression strain amount increment
ntolstep = 300; % Number of total simulation steps

b = [-pconf0; 0; comp_incr; 0; 0; 0]; % RHS of the non-linear equation, epsilon_yy = 0 in plane strain
...
set(gcf, 'Color', 'w');
for i = 1:1:ntolstep
    strain_iter = [0; 0; comp_incr; 0; 0; 0]; % epsilon_yy = 0 in plane strain
    error_tol = 1e-12;
    ...
    while 1 > 0
        ...
        DDSDDE(2,:) = 0; DDSDDE(:, 2) = 0; DDSDDE(2,2) = 1;
    ...
    % Detect bifurcation
    detA = detect_bifur(Cep); % already cubic root
    ...
    if min(detA) < 0 && flag == false
        flag = true;
        semilogy(psideg, 1e3+(detA)); xlabel('Psi, degree'); ylabel('$\sqrt[3]{{\rm det}(A^{ep})}$','Interpreter','latex'); % an offset of 10^3
    end
