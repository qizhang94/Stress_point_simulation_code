function [STRESS, hsv, DDSDDE, DELAS, Cep]=AMCC_UMAT_bifurcation(PROPS, STRESS0, DSTRAIN0, hsv0) %#codegen
    ...
    for j = 1:3
        for k = 1:3
            for l = 1:3
                ...
                Ce(i, j, k, l) = 4270*(i==j)*(k==l) + 2*9360*I4(i, j, k, l) ...
                    + (-1870)*((i==j)*m(k, l) + m(i, j)*(k==l)) + 5420*(m(i, j)*m(k, l)) ...
                    + 2*(6510 - 9360)*(m(i, k)*(j==l) + (i==k)*m(j, l)); % MPa
